...
def add_technical_indicators(data):
    # Ensure the 'Close' column has valid data before calculating indicators
    if 'Close' in data.columns and not data['Close'].empty:
        data['SMA_20'] = ta.trend.sma_indicator(data['Close'], window=20)
        data['EMA_20'] = ta.trend.ema_indicator(data['Close'], window=20)
    else:
        st.warning(
            "The 'Close' column is empty or not available. Technical indicators can't be calculated.")
    return data
...
if st.sidebar.button('Update'):
    ...
    # Check if data is returned and non-empty
    if not data.empty:
        ...
        st.error(
            "No data found for the given ticker and time period. Please check your inputs.")
